...
class AlunosRepository {
  ...
  Future<Aluno> findById(String id) async {
    final alunoResponse =
        await http.get(Uri.parse('http://127.0.0.1:3031/alunos/$id'));
  ...
  Future<void> update(Aluno aluno) async {
    await http.put(Uri.parse('http://127.0.0.1:3031/alunos/${aluno.id}'),
        body: aluno.toJson(),
        headers: {
  ...
  Future<void> insert(Aluno aluno) async {
    await http.post(Uri.parse('http://127.0.0.1:3031/alunos'),
        body: aluno.toJson(),
        headers: {
          'content-type': 'application/json',
        });
